...
def experiment_hyperparameter_gridsearch(seed_set_size=10, top_ps=[0.1, 0.01, 0.001], temperatures=[0, 1, 2], seed_set_generation_methods=[random_split,uniform_split]):
    ...
    for top_p in top_ps:
        for temperature in temperatures:
            for seed_set_generation_method in seed_set_generation_methods:

                seed_labels =_get_seed_instances(seed_set_size, seed_set_generation_method=seed_set_generation_method)

                _generate_labels(query_method=_query_GPT_json, temperature=temperature, top_p=top_p)
                
                accuracy = _evaluate_accuracy(seed_labels, convert_to_indices=False)
                
                print(f"Accuracy for json enforced prompt for method {seed_set_generation_method}, with seedset size {seed_set_size}, top_p: {top_p}, temperature: {temperature}: {accuracy*100}%")
...
if __name__ == "__main__":
    ...
    experiment_changing_return_format_json(seed_set_generation_method=random_split, seed_set_size=100)
